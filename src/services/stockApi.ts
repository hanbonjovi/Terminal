...
async function fetchFromYahoo(symbols: string[]): Promise<StockQuote[]> {
  ...
  const url = `${CORS_PROXY}${encodeURIComponent(
    `${YAHOO_BASE}?symbols=${symbolStr}&fields=regularMarketPrice,regularMarketChange,regularMarketChangePercent,marketCap,regularMarketVolume,regularMarketDayHigh,regularMarketDayLow,regularMarketOpen,regularMarketPreviousClose,fiftyTwoWeekHigh,fiftyTwoWeekLow,shortName,longName`
  )}`
